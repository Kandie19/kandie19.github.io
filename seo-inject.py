...
def process_page(filename: str, meta: dict) -> None:
    ...
    if "<title>" not in head.lower(): head = f"<title>{meta['title']}</title>" + head
...
def process_redirect(filename: str, target: str) -> None:
    ...
    if 'name="robots"' not in html.lower(): html = html.replace("</head>", '<meta name="robots" content="noindex,follow">\n</head>', 1)
    path.write_text(html, encoding="utf-8")

for filename, meta in PAGES.items(): process_page(filename, meta)
for filename, target in REDIRECTS.items(): process_redirect(filename, target)
