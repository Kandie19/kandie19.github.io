...
class HeadParser(HTMLParser):
    def __init__(self):
        super().__init__(); self.title=""; self.in_title=False; self.meta=[]; self.links=[]; self.jsonld=[]; self._jsonld=False; self._buf=[]; self.h1_count=0; self.images=[]
    def handle_starttag(self, tag, attrs):
        attrs=dict(attrs); tag=tag.lower()
        if tag=="title": self.in_title=True
        elif tag=="meta": self.meta.append(attrs)
        elif tag=="link": self.links.append(attrs)
        elif tag=="script" and attrs.get("type","").lower()=="application/ld+json": self._jsonld=True; self._buf=[]
    ...
    def handle_endtag(self, tag):
        ...
        elif tag=="script" and self._jsonld: self.jsonld.append("".join(self._buf).strip()); self._jsonld=False; self._buf=[]
...
def refresh_target(html:str)->str:
    # Accept meta attributes in either common order and both quoted/unquoted URL values.
    match=re.search(r'<meta\b[^>]*http-equiv=["\']?refresh["\']?[^>]*content=["\']?\s*0\s*;\s*url=([^"\' >]+)',html,re.I)
    if not match:
        match=re.search(r'<meta\b[^>]*content=["\']?\s*0\s*;\s*url=([^"\' >]+)["\']?[^>]*http-equiv=["\']?refresh["\']?',html,re.I)
...
def normalized_target(target:str)->str:
    ...
    if parsed.scheme or parsed.netloc:return parsed.path.rstrip("/") or "/"
    return "/"+target.lstrip("/").rstrip("/")

def is_redirect_page(html:str)->bool:return bool(refresh_target(html))

def audit_file(path:Path):
    html=path.read_text(encoding="utf-8",errors="replace"); parser=HeadParser(); parser.feed(html); issues=[]; warnings=[]
    title=parser.title.strip(); description=meta_value(parser,name="description"); robots=meta_value(parser,name="robots").lower(); canon=canonical(parser); redirect=is_redirect_page(html); legacy_target=LEGACY.get(path.name)
    if redirect:
        if "noindex" not in robots: issues.append("redirect page missing noindex")
        if legacy_target:
            expected=BASE+legacy_target
            if not canon or canon.rstrip("/")!=expected.rstrip("/"): issues.append(f"redirect canonical should target {legacy_target}")
            target=normalized_target(refresh_target(html))
            if target!=legacy_target.rstrip("/"): issues.append(f"redirect target should be {legacy_target}")
...
def main():
    files=sorted(p for p in ROOT.rglob("*.html") if ".git" not in p.parts and p.name not in IGNORE); total_issues=total_warnings=0
    print(f"SEO audit root: {ROOT.resolve()}")
    for path in files:
        issues,warnings=audit_file(path); total_issues+=len(issues); total_warnings+=len(warnings); print(f"[{'PASS' if not issues else 'FAIL'}] {path.relative_to(ROOT)}")
